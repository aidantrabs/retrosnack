<script lang="ts">
  ...
  async function loadSquareSDK(cfg: PaymentConfig) {
    ...
    script.src = cfg.environment === 'sandbox'
    ...
    <div class="grid md:grid-cols-5 gap-10">
      <form onsubmit={handleSubmit} class="md:col-span-3 space-y-6">
        ...
        <button
          ...
          class="w-full bg-ink text-sand px-6 py-3 rounded-full text-sm font-medium transition-colors {submitting || !cardReady ? 'opacity-60 cursor-not-allowed' : 'hover:bg-ink/85'}"
        >
          {#if submitting}
            processing...
          {:else if !cardReady}
